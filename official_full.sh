...
#
# 隔离红线：
#   - 禁止写入 /opt/ros、/usr/local
#   - 禁止 apt 安装 ros-* / python3-ros* / libboost-*-dev
#   - 唯一允许的系统工具：g++ cmake make python3 curl tar（及可选 git）
#   - 本机无 Python 头文件时，允许 apt-get download（只下 .deb 再解压，不 apt install）
#
# 日志：official_full_logs/build.log 以及 official_full_logs/01_python.log ...
# =============================================================================
set -euo pipefail
...
SRC="${ROOT}/src"
INSTALL="${ROOT}/official_full_install"
DOC="${ROOT}/doc/official_full"                 # runtime 里 README / run.sh 的源
RUNTIME="${ROOT}/official_full_runtime"
LOG_DIR="${ROOT}/official_full_logs"
BUILD_DIR="${ROOT}/official_full_build"
...
setup_python() {
  ...
  if python3 -c "import catkin_pkg, em, yaml, rospkg" >/dev/null 2>&1; then
    log "skip python pip deps (already importable)"
  else
...
write_final_tree() {
  (cd "${ROOT}" && find . \
    \( -path './.git' -o -path './official_full_logs' -o -path './official_full_build' \) -prune \
...
# runtime 里的 README / run.sh 不复制，只链到 doc/official_full/。
# 相对路径必须从 official_full_runtime/ 出发，所以是 ../doc/official_full/...
link_doc() {
  local name="$1"
  local src="${DOC}/${name}"
  [[ -e "${src}" ]] || fail "缺少 ${src}"
  ln -sfn "../doc/official_full/${name}" "${RUNTIME}/${name}"
}
...
# -----------------------------------------------------------------------------
# package：从 install 抽出可拷走的 official_full_runtime/
# -----------------------------------------------------------------------------
...
cmd_package() {
  ...
  [[ -x "${INSTALL}/bin/rosversion" ]] || fail "缺少 ${INSTALL}/bin/rosversion，请先成功完成: $0 build"
  ...
  rm -rf "${RUNTIME}"
  mkdir -p "${RUNTIME}/bin" "${RUNTIME}/lib" "${RUNTIME}/python" "${RUNTIME}/share"
  # roslaunch/rosrun 只在带 .catkin 的 CMAKE_PREFIX_PATH 里找 lib/<pkg>/<node>
  if [[ -f "${INSTALL}/.catkin" ]]; then
    cp -a "${INSTALL}/.catkin" "${RUNTIME}/.catkin"
  else
    : > "${RUNTIME}/.catkin"
  fi

  copy_bin_script roscore
  copy_bin_script rosmaster
  copy_bin_script rosrun
  copy_bin_script rosnode
  copy_bin_script rosversion
  copy_bin_script rospack
  ...
  copy_share_pkg roscpp || true
  copy_share_pkg std_msgs || true
  copy_share_pkg rosgraph_msgs || true
  copy_share_pkg bond || true
  copy_share_pkg pluginlib || true
  copy_share_pkg class_loader || true
  copy_share_pkg rospack || true

  if [[ -d "${INSTALL}/etc/ros" ]]; then
    mkdir -p "${RUNTIME}/etc"
    cp -a "${INSTALL}/etc/ros" "${RUNTIME}/etc/"
  fi

  # ROS 生成的 msg/srv 与工具包（含 roscpp，rosnode/rospy 会 import）
  if [[ -d "${INSTALL}/lib/python3/dist-packages" ]]; then
    cp -a "${INSTALL}/lib/python3/dist-packages/." "${RUNTIME}/python/"
  fi
  [[ -d "${RUNTIME}/python/rosmaster" ]] || fail "缺少 python 包 rosmaster"
  [[ -d "${RUNTIME}/python/rospy" ]] || fail "缺少 python 包 rospy"
  [[ -d "${RUNTIME}/python/roscpp" ]] || fail "缺少 python 包 roscpp"
  [[ -d "${RUNTIME}/python/roslaunch" ]] || fail "缺少 python 包 roslaunch"
  copy_python_pkg rospkg || fail "缺少 python 包 rospkg"
  copy_python_pkg catkin_pkg || true
  copy_python_pkg yaml || true
  copy_python_pkg defusedxml || true
  copy_python_pkg distro || true
  copy_python_pkg dateutil || true
  copy_python_pkg six || true
  copy_python_pkg packaging || true
  # Python 3.12 无 distutils，sitecustomize 靠 setuptools 补回（rosclean/roslaunch 需要）
  copy_python_pkg setuptools || fail "缺少 python 包 setuptools"
  copy_python_pkg pkg_resources || true
  copy_python_pkg _distutils_hack || true
  if [[ -f "${INSTALL}/lib/python${PYVER}/site-packages/distutils-precedence.pth" ]]; then
    cp -a "${INSTALL}/lib/python${PYVER}/site-packages/distutils-precedence.pth" "${RUNTIME}/python/"
  fi
  if [[ -f "${INSTALL}/lib/python${PYVER}/site-packages/six.py" ]]; then
    cp -a "${INSTALL}/lib/python${PYVER}/site-packages/six.py" "${RUNTIME}/python/"
  fi
