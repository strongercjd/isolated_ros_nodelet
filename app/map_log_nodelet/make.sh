#!/usr/bin/env bash
# =============================================================================
# app/map_log_nodelet/make.sh — SLAM 建图日志记录插件（无 catkin）
# -----------------------------------------------------------------------------
#   ./make.sh x86       编译 libmap_log_nodelet.so → build/
...
cmd_x86() {
  prepare_env
  echo "==== map_log_nodelet x86 build ===="
  mkdir -p "${BUILD}"
  ...
  echo "DONE: ${so}"
  echo "下一步: $0 install（并确认 app_runtime/plugins.json 已注册 ${SO_NAME}）"
}
...
cmd_install() {
  ...
  grep -q '"libmap_log_nodelet.so"' "${RUNTIME}/plugins.json" \
    || echo "WARN: ${RUNTIME}/plugins.json 未注册 ${SO_NAME}，请手工添加 /map_log 条目"
}
