#!/usr/bin/env bash
# =============================================================================
# app/fastbuild_task_nodelet/make.sh — 快建任务节点（无 catkin）
# -----------------------------------------------------------------------------
#   ./make.sh x86       编译 libfastbuild_task_nodelet.so → build/
...
SO_NAME="libfastbuild_task_nodelet.so"
...
cmd_x86() {
  prepare_env
  echo "==== fastbuild_task_nodelet x86 build ===="
  mkdir -p "${BUILD}"
...
cmd_install() {
  ...
  if ! grep -q "${SO_NAME}" "${RUNTIME}/plugins.json" 2>/dev/null; then
    echo "WARN: ${RUNTIME}/plugins.json 尚未注册 fastbuild_task，请手工加入以下条目:" >&2
    cat >&2 <<'EOF'
    {
      "name": "/fastbuild_task",
