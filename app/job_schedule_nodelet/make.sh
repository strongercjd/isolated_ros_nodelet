#!/usr/bin/env bash
# =============================================================================
# app/job_schedule_nodelet/make.sh — 任务调度节点（无 catkin）
# -----------------------------------------------------------------------------
#   ./make.sh x86       编译 libjob_schedule_nodelet.so → build/
...
SO_NAME="libjob_schedule_nodelet.so"
...
cmd_x86() {
  prepare_env
  echo "==== job_schedule_nodelet x86 build ===="
  mkdir -p "${BUILD}"
...
cmd_install() {
  ...
  if ! grep -q "${SO_NAME}" "${RUNTIME}/plugins.json" 2>/dev/null; then
    echo "WARN: ${RUNTIME}/plugins.json 尚未注册 job_node，请手工加入以下条目:" >&2
    cat >&2 <<'EOF'
    {
      "name": "/job_node",
