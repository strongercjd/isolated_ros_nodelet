#!/usr/bin/env bash
# =============================================================================
# app/base_navi_nodelet/make.sh — 基础导航节点（通用导航规划，无 catkin）
# -----------------------------------------------------------------------------
#   ./make.sh x86       编译 libbase_navi_nodelet.so → build/
...
SO_NAME="libbase_navi_nodelet.so"
...
cmd_x86() {
  prepare_env
  echo "==== base_navi_nodelet x86 build ===="
  mkdir -p "${BUILD}"
...
cmd_install() {
  ...
  if ! grep -q "${SO_NAME}" "${RUNTIME}/plugins.json" 2>/dev/null; then
    echo "WARN: ${RUNTIME}/plugins.json 尚未注册 base_navi，请手工加入以下条目:" >&2
    cat >&2 <<'EOF'
    {
      "name": "/base_navi",
